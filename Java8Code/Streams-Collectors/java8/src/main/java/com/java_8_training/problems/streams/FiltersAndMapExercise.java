...
public class FiltersAndMapExercise {
    ...
    public static  List<Integer> findEvenNumbers() {
        ...
        evenNumbers = input.filter((Integer i) -> { if(i%2 == 0) return true; else return false;})
                .collect(toList());
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter((String n)-> name.equals(n))
             .collect(toList());
    ...
    public static List<Integer> filterNull(){
        ...
        List<Integer> evenNumbers = new ArrayList<>();
        evenNumbers = input.filter((Integer n) -> (n != null))
                            .collect(toList());
        return evenNumbers;
    }
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = personList.stream()
                        .filter((p) -> p.equals(name) && p.equals(age))
                        .collect(toList());
    ...
    public static List<Integer> flatteningLists() {
        ...
        together = input.flatMap(elem -> elem.stream())
                        .collect(toList());
    ...
    public static boolean areAllPersonsBelowAge(List<Person> personList, Integer age){
        ...
        overAge = personList.stream()
                            .allMatch(p->p.getAge()<70);
    ...
    public static int findTheMinimumNumber(){
        Stream<Integer> input =  Stream.of(5, 2, 200, 33, 150, 4);
        //TODO #11
        // HINT: use reduce
        int min =0;

        input.reduce(Integer.MAX_VALUE, Integer::min);
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        min = personList.stream()
                .collect(minBy(Comparator.comparingInt(Person::getAge))).get().getAge();
        return min;
    }
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream().filter(e ->  (e %3 == 0 && Math.pow(Math.sqrt(e), 2) == e) )
                .findFirst();
