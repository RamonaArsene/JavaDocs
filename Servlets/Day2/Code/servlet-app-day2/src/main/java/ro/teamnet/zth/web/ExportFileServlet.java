...
//                .flatMap(elem -> Stream.of(paramsMap.put(elem.toString(), request.getParameter(elem.toString()))))
//                .collect(Collectors.toMap(Enumeration::toString, elem->request.getParameter(elem.toString())));
        ...
        try (InputStream reportStream = getClass().getResourceAsStream(jasperPath)) {
            ...
            response.setHeader("filename", passedTemplate+ "." +passedFileType);
